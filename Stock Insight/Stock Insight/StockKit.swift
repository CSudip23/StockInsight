...
class StockKit{
    ...
    class func fetchStocksFromSearchTerm(term: String, completion:@escaping(_ stockInfoArray: [StockSearchResult]) -> ()) {
        DispatchQueue.global(qos: .background).async() {
            ...
            AF.request(searchURL, method: .get, parameters: ["query": term, "region": 2, "lang": "en"]).responseJSON { response in

//            Alamofire.request(searchURL, method: .get, parameters: ["query": term, "region": 2, "lang": "en"]).responseJSON { response in
                
                if let resultJSON = response.value as? [String : AnyObject]  {
    ...
    //            "https://api-v2.intrinio.com/companies/AAPL?api_key=OjkxNGVjOGNiNWI4YmRlODk2NGQwNjFhYjQ0N2ZkYWI5"
    //            "http://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20yahoo.finance.quotes%20where%20symbol%20in%20(%22\(symbol)%22)&env=store%3A%2F%2Fdatatables.org%2Falltableswithkeys&format=json"
                        
                AF.request(stockURL, method: .get).responseJSON { response in
    ...
    class func fetchChartPoints(symbol: String, range: ChartTimeRange, completion:@escaping(_ chartPoints: [ChartPoint]) -> ()) {
          ...
          DispatchQueue.global(qos: .background).async(){
              ...
              let chartURL = StockKit.chartUrlForRange(symbol: symbol, range: range)
              
              AF.request(chartURL, method:.get ).responseData { response in
...
class SwiftStockChart: UIView {
    ...
    func setChartPoints(points: [ChartPoint]) {
        ...
        //(var i = 0; i < verticalGridStep!; i++)
        {
            ...
            let p = CGPoint(x: valueLabelPosition! == .Right ? axisWidth! : 0, y: yVal)
                //CGPointMake((valueLabelPosition! == .Right ? axisWidth! : 0), yVal)
        ...
        for i in 0..<horizontalGridStep!
            //(var i = 0; i < horizontalGridStep! + 1; i++)
        {
    ...
    func drawGrid() {
        ...
        if drawInnerGrid! {
            ...
            for i in 0..<verticalGridStep!
                //var i = 0; i < verticalGridStep! + 1; i++
            {
...
//        CGPointMake(xValue,yValue)
    ...
    func timeLabelsForTimeFrame(range: ChartTimeRange) -> [String]
    {
        ...
        case .FiveDays:
            ...
            let weekday = NSCalendar(calendarIdentifier: NSCalendar.Identifier.gregorian)!.components(.weekday, from: NSDate() as Date).weekday
           switch weekday {
           case 1:
        ...
        case .OneMonth:
            ...
            let start = NSDate()
            let fifthString = fmt.string(from: start.addingTimeInterval(offset) as Date)
            let fourthString = fmt.string(from: start.addingTimeInterval(offset * 2) as Date)
            let thirdString = fmt.string(from: start.addingTimeInterval(offset * 3) as Date)
            let secondString = fmt.string(from: start.addingTimeInterval(offset * 4) as Date)
            let firstString = fmt.string(from: start.addingTimeInterval(offset * 5) as Date)
